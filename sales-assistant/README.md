...
LinkedIn上の人物情報を手動で登録すると、Google Gemini APIが株式会社ENとの相性を判定し、
...
4. **相性判定** — Geminiが総合スコア／顧客候補スコア／協業候補スコア／優先度(A/B/C)／相性理由／相手の課題／ENの提供価値／話題候補／見送り理由を判定。情報不足時は「不足情報」「追加すべき項目」「汎用文面への警告」を返す設計。
...
Next.js (App Router) / TypeScript / Tailwind CSS / shadcn/ui相当の自作コンポーネント / Prisma + PostgreSQL / Google Gemini API（@google/genai、AIプロバイダー層で抽象化） / bcryptjs（パスワードハッシュ） / Zod / Vitest
...
# （GEMINI_API_KEY は任意。設定しなくてもモックモードで動作します）
...
`GEMINI_API_KEY` が未設定の場合でも、Geminiを呼び出す代わりに軽量なテンプレートで結果を生成する
**モックモード**で自動的に動作します。相性判定・つながり申請文/初回DM生成・文面チェックを含む
一連の画面操作をAPIキーなしで確認できます（画面上部と各結果に「モック生成」であることを明示するバナー/バッジを表示します）。

### AIプロバイダー層

AI呼び出しは `src/lib/ai/provider.ts` の `AiProvider` インターフェース経由で行っており、
現在の実装は `src/lib/ai/providers/gemini.ts`（Gemini API, `@google/genai`使用）のみです。
将来別のプロバイダー（例: OpenAI）へ切り替える場合も、`ai-service.ts` 以降のコードは一切変更せず、
`src/lib/ai/index.ts` の `getAiProvider()` が返す実装を差し替えるだけで対応できます。

- レート制限（無料枠の上限）に達した場合は、1回だけ自動リトライしたうえで
  「Gemini APIの無料利用上限に達しました。時間を置いて再実行してください」という
  エラーメッセージを画面に表示します。
- 候補者を複数件処理する場合（例: メッセージ3パターン分の文面チェック）は、常に同時実行せず1件ずつ順番に処理します。
- プロフィール本文等の入力内容や生成結果は、エラーログに一切出力しません。
...
上記の準備（PostgreSQL対応・ログイン機能・`vercel-build`スクリプト・Gemini APIプロバイダー等）は
関連するPull Requestにすべて含まれています。手順3で連携する際は、対象のPRがマージされたあと
（またはそのブランチを対象に）Vercel側でプレビューデプロイを作成する形でも確認できます。
...
- メッセージ生成のたびに Gemini を複数回（生成1回＋3パターンぶんの文面チェック3回 = 最大4回、順番に実行）呼び出すため、応答時間がやや大きくなります。無料枠のレート制限に達しやすい点にも注意してください。
...
2. Gemini呼び出し回数を減らすオプション（文面チェックをスキップして生成のみ行うトグル等）。
