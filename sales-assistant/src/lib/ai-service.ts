import { callAiJsonTool, getAiModelId, isAiConfigured } from "./ai";
import {
  ANALYSIS_TOOL_NAME,
...
export async function analyzeLeadCompatibility(
  ...
  const raw = await callAiJsonTool({
    system,
    prompt,
  ...
  return { result, raw, mocked: false, modelUsed: getAiModelId() };
}
